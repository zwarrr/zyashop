...
<body class="bg-white text-black">
  ...
  <main class="max-w-md mx-auto px-4 pt-4">
    ...
    <div class="flex justify-end mb-6">
      ...
      >
        <img src="<?php echo e(asset('img/share.svg')); ?>" alt="Share" class="w-5 h-5">
      </button>
    </div>

    <!-- Profile Section -->
    <section class="text-center mb-6">
      <img src="<?php echo e($userProfile?->profile_image ?? 'https://i.pinimg.com/736x/78/0d/08/780d084f353d666f61a0067dbf48bfdd.jpg'); ?>" alt="Profile" class="w-28 h-28 mx-auto rounded-full bg-gray-300 mb-4" />

      <h1 class="text-[20px] font-bold leading-tight"><?php echo e($userProfile?->display_name ?? 'User'); ?></h1>
      <p class="text-gray-700 text-sm">
  <?php echo e('@' . ($userProfile?->username ?? 'thezyshop')); ?>

  <?php if($userProfile?->verified_badge === 'yes'): ?>
  <img src="<?php echo e(asset('img/verift.svg')); ?>" alt="Verified" class="inline w-4 h-4 ml-1 mb-[2px]">
  <?php endif; ?>
      </p>
      <p class="text-gray-500 text-sm mt-[6px]"><?php echo e($userProfile?->bio ?? 'We sell digital content packages daily!'); ?></p>

      <!-- External Links Container -->
      <?php if($userLinks && $userLinks->count() > 0): ?>
      <div class="mt-3 flex items-center justify-center">
        ...
        <button id="mainLinkBtn" class="main-external-link flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium text-black transition-colors border-none bg-transparent cursor-pointer" style="display: none;">
          <img src="<?php echo e(asset('img/link.svg')); ?>" alt="Link" class="w-4 h-4 flex-shrink-0" />
          <span class="flex items-center gap-0.5 whitespace-nowrap">
            <span class="link-display"><?php echo e($userLinks->first()->url); ?></span>
      ...
      <div id="allLinksContainer" style="display: none;">
        ...
        <a href="<?php echo e($link->url); ?>" class="external-link"><?php echo e($link->title); ?></a>
    ...
    <section class="grid grid-cols-2 gap-x-4 gap-y-5 mb-20">
      ...
      <a href="javascript:void(0)" 
         onclick="handleCardClick(<?php echo e($card->id); ?>, '<?php echo e($card->title); ?>', <?php echo e($card->products()->where('status', '!=', 'inactive')->count()); ?>)" 
         class="card-link relative rounded-lg overflow-hidden group cursor-pointer" 
         data-card-id="<?php echo e($card->id); ?>" 
         data-has-products="<?php echo e($card->products()->where('status', '!=', 'inactive')->count() > 0 ? 'true' : 'false'); ?>"
         title="<?php echo e($card->title); ?>">
        <?php if($card->image_url): ?>
          <img src="<?php echo e($card->image_url); ?>" 
               alt="<?php echo e($card->title); ?>" 
               class="w-full h-full object-cover"
               onerror="this.src='https://placehold.co/1080x1080?text=<?php echo e(urlencode($card->title)); ?>'">
        <?php else: ?>
          <img src="https://placehold.co/1080x1080?text=<?php echo e(urlencode($card->title)); ?>" 
               alt="<?php echo e($card->title); ?>" 
               class="w-full h-full object-cover">
        <?php endif; ?>
        <div class="absolute bottom-0 w-full bg-black/80 text-white text-xs text-center py-2 group-hover:bg-black/90 transition-all"><?php echo e($card->title); ?></div>
      </a>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </section>
    <?php else: ?>
  ...
  <!-- Load Links Modal -->
  <?php echo $__env->make('partials.modal_link_profile', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
  ...
  <script>
    ...
    function openLinksModal(linksData = []) {
      ...
      linksData.forEach(link => {
        ...
        linkItem.innerHTML = `
          <img src="<?php echo e(asset('img/link.svg')); ?>" alt="Link" class="link-icon" />
...
<?php /**PATH C:\GOW\zyashop\resources\views/zyashp.blade.php ENDPATH**/ ?>