      ...
      <div class="flex items-start gap-6">
        ...
        <div class="flex-1">
          <div class="mb-4">
            ...
            <p class="text-sm text-gray-700">{{ '@' . ($profile?->username ?? 'thezyshop') }}</p>
          </div>
          ...
          <div class="mb-4">
            ...
            <p class="text-lg font-semibold text-gray-800 flex items-center gap-2">
              ...
              @if($profile?->verified_badge === 'yes')
              <img src="{{ asset('img/verift.svg') }}" alt="Verified" class="w-5 h-5">
              @endif
            </p>
          </div>
          ...
          <div>
            ...
            <p class="text-sm text-gray-700">{{ $profile?->bio ?? 'We sell digital content packages daily!' }}</p>
          </div>
        </div>
      </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            ...
            <tr class="hover:bg-gray-50 transition">
              <td class="px-4 py-4 text-sm font-medium text-gray-900">{{ $link->title }}</td>
              <td class="px-4 py-4 text-sm text-gray-600">
                <a href="{{ $link->url }}" target="_blank" class="text-blue-600 hover:underline">{{ Str::limit($link->url, 50) }}</a>
              </td>
              <td class="px-4 py-4 text-sm text-gray-600">{{ $link->order }}</td>
              <td class="px-4 py-4 text-sm text-center relative">
                <div class="inline-block text-left">
                  <button onclick="toggleDropdown({{ $link->id }})" class="p-2 hover:bg-gray-100 rounded-lg transition duration-200" title="Aksi">
                  ...
                  <div id="dropdown-{{ $link->id }}" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-200 z-50">
                    <button onclick="openLinkModal('edit', {{ $link->id }}); toggleDropdown({{ $link->id }})" class="w-full flex items-center gap-2 px-4 py-3 text-gray-700 hover:bg-gray-50 text-sm transition duration-200 text-left rounded-t-lg">
                    ...
                    <button onclick="deleteLink({{ $link->id }}); toggleDropdown({{ $link->id }})" class="w-full flex items-center gap-2 px-4 py-3 text-red-600 hover:bg-red-50 text-sm transition duration-200 text-left rounded-b-lg">
        ...
        <div class="space-y-4">
          <div>
            ...
            <div class="flex items-center gap-2">
              ...
              <input type="text" name="username" id="username" value="{{ $profile?->username ?? 'thezyshop' }}" required 
          ...
          <div>
            ...
            <input type="text" name="display_name" id="displayName" value="{{ $profile?->display_name ?? '' }}" required 
          ...
          <div>
            ...
            <textarea name="bio" id="bio" rows="4" 
                      class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-black">{{ $profile?->bio ?? '' }}</textarea>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div>
            ...
            <select name="verified_badge" id="verifiedBadge" 
                    class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-black">
              <option value="no" {{ ($profile?->verified_badge ?? 'no') === 'no' ? 'selected' : '' }}>No</option>
              <option value="yes" {{ ($profile?->verified_badge ?? 'no') === 'yes' ? 'selected' : '' }}>Yes</option>
            </select>
          </div>
...
<script>
  ...
  document.getElementById('linkForm').addEventListener('submit', async (e) => {
    ...
    let url = '{{ route("profile.links.store") }}';
    const formData = new FormData(e.target);
